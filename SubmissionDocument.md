...
For our algorithms we created doc2vec neural networks for each method to heighten their importance for each method when doing comparisons. When given a specific query, we run thorugh every topic and compare the query with the topic's associated method using the topic's asssociated neural mode. The topic that reutnrs the highest similarity score is returned as the assuemd method of the query. 
